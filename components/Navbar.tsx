...
import { Phone, Mail, Search, ChevronDown, ChevronUp, Menu, X, BookOpen, Users, Stethoscope, GraduationCap } from 'lucide-react';
    ...
    <header className={`sticky top-0 left-0 w-full z-[9999] flex flex-col bg-white transition-shadow duration-300 ${scrolled ? 'shadow-[0_4px_20px_rgba(0,0,0,0.08)]' : ''}`}>
      ...
      >
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end shrink-0">
          ...
          <Link href="/register" className="bg-mf-red hover:bg-mf-red-light hover:-translate-y-px text-white font-semibold px-4 py-1.5 rounded-full transition-all">
            Register Now
          </Link>
        </div>
      </div>
      ...
      {/* Main Navbar */}
      <nav className="relative z-[9999] bg-white border-b border-gray-100 shadow-sm">
        <div className="max-w-[1440px] mx-auto px-4 md:px-8 py-2 flex items-center justify-between">

          {/* Left: Logo & CPD */}
          <div className="flex items-center shrink-0">
            <Link href="/" onClick={() => setMenuOpen(false)} aria-label="Go to home" className="flex items-center decoration-none shrink-0">
              <img src="/logo.png" alt="MOHAN Foundation" className="h-[46px] w-auto object-contain shrink-0" />
            </Link>

            {/* Divider & CPD Badge (Desktop/Tablet) */}
            <div className="hidden md:flex items-center shrink-0 ml-6 pl-6 lg:ml-8 lg:pl-8 border-l-[1.5px] border-gray-200/60 self-stretch">
              <a href="https://directory.cpdstandards.com/providers/mohan-foundation/" target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 shrink-0 hover:opacity-85 hover:scale-[1.01] transition-all duration-300 group cursor-pointer">
                <img src="/cpd-badge-new.png" alt="CPD Badge" className="w-16 h-16 object-contain shrink-0 mix-blend-multiply transition-all duration-300" />
                <div className="flex flex-col justify-center">
                  <span className="text-[18px] font-bold leading-tight text-[#1E2548]">CPD Standards Office</span>
                  <span className="text-[14px] leading-tight text-sky-500 mt-[2px]">The Educator's Trustmark</span>
                </div>
              </a>
            </div>
          </div>
          ...
          <button
            className="lg:hidden text-mf-navy hover:text-mf-red transition-colors p-2 shrink-0 ml-auto"
          ...
          {/* Desktop Navigation & CTA (>= 1024px) */}
          <div className="hidden lg:flex items-center shrink-0">
            <ul className="flex items-center gap-1 xl:gap-2 text-[0.85rem] font-medium text-mf-navy mr-6">
              {navLinks.slice(0, 2).map(({ label, href }) => (
                <li key={label}>
                  <Link
                    href={href}
                    className="px-3 py-2 rounded-full transition-all flex items-center hover:text-mf-red hover:bg-mf-red/5"
                  >
                    {label}
                  </Link>
                </li>
            ...
            <Link href="/courses" className="bg-mf-red hover:bg-mf-red-light text-white px-6 py-2.5 rounded-full font-semibold text-[0.85rem] transition-all hover:-translate-y-px hover:shadow-[0_8px_25px_rgba(200,48,58,0.35)] whitespace-nowrap shrink-0">
              Explore Courses
            </Link>
          </div>
        </div>
